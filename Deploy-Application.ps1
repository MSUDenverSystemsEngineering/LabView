...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'labview,nimax,ultiboard,vi package manager,niceipsettings,nimxs,nimdnsresponder,nierserver,nidiscsvc,applicationwebserver,systemwebserver,tagsrv,nierserver,nidmsrv,nisvcloc,niauth_daemon,niwebservicecontainer' -PromptToSave -PersistPrompt -CheckDiskSpace

		## <Perform Pre-Installation tasks here>
		$NETFrameworkVersion = Get-RegistryKey -Key 'HKLM:SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Client' -Value 'Release'

		If (Test-Path -LiteralPath (Join-Path -Path "$envProgramFilesX86\SMART Technologies" -ChildPath "Education Software\TeacherTools.exe") -PathType 'Leaf') {
			Write-Log -Message 'SMART Response was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
			Show-InstallationProgress -StatusMessage 'Uninstalling SMART Response software...'
			Remove-MSIApplications -Name 'SMART Response Software'
		}

		If (Test-Path -LiteralPath (Join-Path -Path "$envProgramFiles\National Instruments\Shared" -ChildPath "NIUninstaller\uninst.exe") -PathType 'Leaf') {
			...
			Execute-Process -Path "$envProgramFiles\National Instruments\Shared\NIUninstaller\uninst.exe" -Parameters '/qb /x all /log ni.txt' -WindowStyle 'Hidden' -IgnoreExitCodes '3010'
		}
		If (Test-Path -LiteralPath (Join-Path -Path "$envProgramFilesx86\National Instruments\Shared" -ChildPath "NIUninstaller\uninst.exe") -PathType 'Leaf') {
			Write-Log -Message 'National Instruments software was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
			Show-InstallationProgress -StatusMessage 'Uninstalling previous versions of National Instruments software...'
			Execute-Process -Path "$envProgramFilesx86\National Instruments\Shared\NIUninstaller\uninst.exe" -Parameters '/qb /x all /log ni.txt' -WindowStyle 'Hidden' -IgnoreExitCodes '3010'
		}
		...
		Execute-Process -Path "$dirFiles\setup.exe" -Parameters "specfile.txt /r:n /q /disableNotificationCheck /AcceptLicenses yes /groups `"`""  -WindowStyle 'Hidden' -IgnoreExitCodes '3010'
		...
		Remove-File -Path "$envCommonDesktop\NI Max.lnk"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'labview,nimax,ultiboard,vi package manager,niceipsettings,nimxs,nimdnsresponder,nierserver,nidiscsvc,applicationwebserver,systemwebserver,tagsrv,nierserver,nidmsrv,nisvcloc,niauth_daemon,niwebservicecontainer' -CloseAppsCountdown 60 -PromptToSave -PersistPrompt
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBOe+bUNdVdvQNS
# seVqKqZF7pfUdfyng/tYZfrPIDaFU6CCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIGQD
# yjdYfQy83oKFK2s6IMVHxi9SJ4UxanpH/DckJOPjMA0GCSqGSIb3DQEBAQUABIIB
# AEHErh4oyRO0IWFhTTD3i5vNFzjwh6tVwM4NtO6FvS3Slnpoyix4ldQyJQD/RBBM
# z1tXQo7njTrebY2jrxKj1zwV3QxJJoVt4ZTiektnMNS6S9qURT5/sqOafjpZOg60
# lY4tM/r7ZgDr3mWQ/V2TLFjX4Jg4SoZZkWJiUMSo7ghbrz7VUw3SSmCojJ+z8EJ6
# B9MHl6t03Q4GVnn1MW+OpReQIvVnGJUQjogK1aToowoou1su8OG+trbI3U0NPPqT
# eTcQjDGwOpGiAk0JgpqpdYdGBjqsIKWVRXdtfLUmP+RdGRNXD1xvNdt/utJ2bHVJ
# BaQOv2+KrX5N57gcas4aEquhggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTYxMDEzMjA1MjQwWjAjBgkqhkiG9w0BCQQxFgQU
# sQ+SgZcK4V8Psi0o27E4GWDnVSAwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBAJ62mJ6iyddEVEitYA6MzpTx7+W1/CXOpem6J2qt49cvwGxHrvz+ieYj
# lyAr3GF8Ll+MxYzw0VzH3vvbml5r/DGPkLIHMi9t1akbw1yPi/jTEHTUM+Pv3lUX
# aj7JPKsIo8EM8X9VQ+E89Sv2kK72b+u+ShwdnwvpE2xqG2lbGFXUmXP8If1ipltt
# hce/CLhV7MLjLIUs5Y8Ng8UsDiAWauPiCz6XNDLDNkmpML2FqyoelLDyqIPfC5zo
# Dq/CjqKulOVSIGpVNakWPUfGnsms5qsK7J34ytz84HQ3LVtH2njs/IE5gxFEXsMz
# GVS8bD1a/l+SDr4zNg+z8IXG41J4TmU=
